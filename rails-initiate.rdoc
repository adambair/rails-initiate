...
Past instructors include Adam Bair, Joe Grossberg, and Paul Ostazeski.
...
* macports or brew for system packages
* rbenv or rvm for ruby version management
* or use Intridea's "curl intridea.me" bootstrap package
...
* in this case we're going to use "blog"

    rails new blog

* This will create a directory called 'blog' containing a new rails application

=== Anatomy of a Rails applicatoin

    app     #=> models, views, controllers, helpers, app logic, and assets
    config  #=> database, environment configuration, routes
    db      #=> migrations, schema.rb, database seeds
    ...
    public  #=> static pages, default rails page, favicon, error pages
    script  #=> contains the 'rails' command
    spec    #=> rspec tests/specifications and configuration
    test    #=> built in test suite Test::Unit
    tmp     #=> sessions, cache, etc
    vendor  #=> 3rd party code, plugins, gems, and assets
...
* Development 
  ...
  - Reloads Ruby classes on each request
...
* Environments have separate configurations
  ...
  - config/application.rb is run for every environment

* You can create your own environments (the defaults are conventions)
  - Often you'll see an additional environment called 'staging'
...
=== Starting a Rails application
    ...
    rails server  #=> start app in dev mode at http://localhost:3000
    rails server -p 4000 #=> starts the app at http://localhost:4000
    rails server -e production #=> start the application in production mode
...
* Once you're comfortable with Ruby and Rails you can even create your own
    ...
    rails generate #=> will list all available generators

=== Creating a Scaffold
A Scaffold will create all views, models, controllers, routes, and migrations for a resource

    rails generate scaffold #=> will explain what the generator does
...
* Extremely useful to sketch out or test ideas
    ...
    Post.find_by_title('happy')
                      #=> Finds first (single) post with the title happy

    Post.find_all_by_title('happy')
                      #=> Finds all (array) posts with the title happy
...
* Keep controllers small by moving business logic in your models
...
== Lab 06: Examine Partials and Review [ 06_extract_post_form_fields.rb ]
* Review the partial in app/views/posts/edit.html.erb and new.html.erb
* Review the layout of the rails application
...
* It's common to have only a few layouts for your application
  - examples: application.html.erb (default), site.html.erb, admin.html.erb
...
* Layouts are a good place to render you header and footers
...
* All helpers in app/helpers are available to all of your views
* The naming convention and separate files is for organization

* You can extract logic from your views into helper methods
* These are normal ruby methods that are accessable from your views
...
* Create a route that reflects the post has nested comments
...
* You can protect actions by using the http_basic_authenticate_with method:
...
- For more robust authentication check out omniauth:
...
=== Rails Asset Pipeline for JS and CSS

  Awesome, yet can be slightly confusing.
...
=== Capybara
* A better Cucumber ;)
* Programmer friendly integration/front-end testing
...
=== SASS/SCSS (Syntatically Awesome Stylesheets)
* http://sass-lang.com
